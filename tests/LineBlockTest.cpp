...
int main(int argc, char** argv) {
    ...
    // A section has to stop dead at its own border. This is what made the block signal go
    // to danger in the driver's face: the occupancy tolerance is lateral, but wrapped round
    // the end of a run it read as longitudinal too, and the signal's own section - which
    // begins half a metre beyond the mast - counted a train still standing at the mast.
    std::puts("\nwhere one block section stops and the next begins:");
    {
        // Sample the two blocks' runs over track 586 exactly as the sim samples them.
        auto runOver = [&](int sectionId) {
            std::vector<glm::dvec2> out;
            for (const Section& sec : circuits.sections) {
                if (sec.id != sectionId) continue;
                for (const SectionInterval& iv : sec.parts) {
                    if (iv.trackId != kBlockTrack) continue;
                    for (const glm::dvec3& w : sampleSectionRun(polys, iv))
                        out.push_back({w.x, w.y});
                }
            }
            return out;
        };
        const std::vector<glm::dvec2> runS = runOver(secS), runN = runOver(secN);
        check(runS.size() >= 2 && runN.size() >= 2, "both blocks sampled over the border",
              static_cast<long>(runS.size()), static_cast<long>(runS.size()));
        double len = 0.0;
        for (const TrackPoly& q : polys)
            if (q.id == kBlockTrack) len = polyLength(q.pts);
        constexpr double kTol2 = kOccupancyTolM * kOccupancyTolM;
        auto occupiedAt = [&](const std::vector<glm::dvec2>& run, double metresPastBorder) {
            const glm::dvec3 w = fracToWorld(polys, kBlockTrack,
                                             kBlockFrac + metresPastBorder / len);
            for (std::size_t i = 1; i < run.size(); ++i)
                if (runSegDist2({w.x, w.y}, run[i - 1], run[i], i == 1,
                                i + 1 == run.size()) < kTol2)
                    return true;
            return false;
        };
        // A train standing at the mast, half a metre short of the border, is behind it.
        check(!occupiedAt(runS, 0.5), "a train 0.5 m past the border is out of the block "
              "behind it", occupiedAt(runS, 0.5) ? 1 : 0, 0);
        check(!occupiedAt(runS, 2.0), "and 2.0 m past it too", occupiedAt(runS, 2.0) ? 1 : 0, 0);
        check(occupiedAt(runS, -0.5), "while 0.5 m short of it, it is inside",
              occupiedAt(runS, -0.5) ? 1 : 0, 1);
        // And no gap the other way: whatever leaves one block is in the next.
        check(occupiedAt(runN, 0.5), "the block ahead picks it up at once",
              occupiedAt(runN, 0.5) ? 1 : 0, 1);
        check(!occupiedAt(runN, -0.5), "and does not reach back over the border",
              occupiedAt(runN, -0.5) ? 1 : 0, 0);
        check(occupiedAt(runS, 0.0) && occupiedAt(runN, 0.0),
              "exactly on the border it is in both, never in neither",
              occupiedAt(runS, 0.0) && occupiedAt(runN, 0.0) ? 1 : 0, 1);
    }

    // A train anywhere on the line has to register in the section it is standing in.
    //
    // This is the one that let a train out of Svenningdal and then lost it. The runs were
    // sampled at a fixed 32 steps however long the interval was, so on the plain line the
    // steps were hundreds of metres and their chords cut the curves by far more than an
    // axle is matched within: the block section read clear with a train in it, the line's
    // claim was released behind him, and the block signal ahead went back to danger.
    std::puts("\ncan a section see a train anywhere inside it:");
    {
        double worstEver = 0.0;
        std::string worstWhere;
        int blind = 0;
        for (const Section& sec : circuits.sections) {
            for (const SectionInterval& iv : sec.parts) {
                const std::vector<glm::dvec3> run = sampleSectionRun(polys, iv);
                if (run.size() < 2) continue;
                // Walk the real rails far more finely than the run was sampled, and ask how
                // far each point is from what the occupancy test would match it against.
                double worst = 0.0;
                for (int k = 0; k <= 400; ++k) {
                    const double f = iv.from + (iv.to - iv.from) * k / 400.0;
                    const glm::dvec3 w = fracToWorld(polys, iv.trackId, f);
                    double best = 1e30;
                    for (std::size_t i = 1; i < run.size(); ++i)
                        best = std::min(best, runSegDist2({w.x, w.y},
                                                          {run[i - 1].x, run[i - 1].y},
                                                          {run[i].x, run[i].y}, i == 1,
                                                          i + 1 == run.size()));
                    worst = std::max(worst, std::sqrt(best));
                }
                if (worst >= kOccupancyTolM) ++blind;
                if (worst > worstEver) {
                    worstEver = worst;
                    worstWhere = sec.name;
                }
        ...
        check(blind == 0, "no section anywhere has a blind spot", blind, 0);
        std::printf("    worst miss %.2f m of %.2f m allowed, in \"%s\"\n", worstEver,
                    kOccupancyTolM, worstWhere.c_str());
    }
