...
int main(int argc, char** argv) {
    ...
    // Flag posts: where the TXP's hand signal hangs.
    std::vector<FlagPost> flagPosts = loadFlagPosts(datasetRoot);
    std::vector<SignalStation> flagPostStation;
    ...
    auto rebuildStructs = [&]() {
        ...
        // The flag posts, all with an empty fixture: the editor has no TXP, and what a
        // post would be showing is not what it is being placed for.
        flagPostStation = attachStations(flagPosts, stations, polys);
        {
            FlagMesh fm;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (mode == EdMode::Geometry) {
            ...
            // F: turn it round. Which way it faces is which movements it governs.
            if (kF && !prevF) {
                if (haveSel) {
                    SimpleEntrySignal& e = simpleEntries[selSimple];
                    e.dir = -e.dir;
                    simpleDirty = true;
                    ...
                    pathMsg = "right-click a simple entry signal first, then F";
                }
                pathMsgUntil = glfwGetTime() + 3.0;
                rebuildOverlay();
            }
            if (kX && !prevX && haveSel) {
                simpleEntries.erase(simpleEntries.begin() + selSimple);
                selSimple = -1;
                simpleDirty = true;
                std::printf("[trackedit] simple entry signal removed (Ctrl+S to save)\n");
                rebuildStructs();
                rebuildOverlay();
            }
            ...
            // F swaps which side of the track it stands on. Not a facing: the flag hangs,
            // so it reads from either direction.
            if (kF && !prevF) {
                if (haveSel) {
                    flagPosts[selFlag].side = -flagPosts[selFlag].side;
                    flagDirty = true;
                    pathMsg = std::string("stands ") +
                              (flagPosts[selFlag].side > 0 ? "right" : "left") +
                              " of increasing frac";
                    rebuildStructs();
                } else {
                    pathMsg = "right-click a flag post first, then F";
                }
                pathMsgUntil = glfwGetTime() + 3.0;
                rebuildOverlay();
            }
            if (kX && !prevX && haveSel) {
                flagPosts.erase(flagPosts.begin() + selFlag);
                selFlag = -1;
                flagDirty = true;
                std::printf("[trackedit] flag post removed (Ctrl+S to save)\n");
                rebuildStructs();
                rebuildOverlay();
            }
        ...
        if (kSave && !prevS && mode == EdMode::Circuits &&
            ...
            if (writeFlagPosts(datasetRoot, flagPosts)) {
                std::printf("[trackedit] saved %zu flag post(s) to "
                            "%s/overlay/flag-posts.txt\n", flagPosts.size(),
                            datasetRoot.c_str());
                flagDirty = false;
            } else {
                std::fprintf(stderr, "[trackedit] failed to write flag posts\n");
            }
            ...
            if (writeSimpleEntrySignals(datasetRoot, simpleEntries)) {
                std::printf("[trackedit] saved %zu simple entry signal(s) to "
                            "%s/overlay/simple-entry-signals.txt\n", simpleEntries.size(),
                            datasetRoot.c_str());
                simpleDirty = false;
            } else {
                std::fprintf(stderr, "[trackedit] failed to write simple entry signals\n");
            }
        ...
        if (!g_mouseCaptured && mL && !prevML && mode == EdMode::Geometry) {
            ...
            if (circHit) {
                FlagPost fp;
                fp.id = nextFlagId++;
                fp.name = "FL" + std::to_string(fp.id);
                fp.trackId = circTrack;
                fp.frac = circFrac;
                flagPosts.push_back(std::move(fp));
                selFlag = static_cast<int>(flagPosts.size()) - 1;
                flagDirty = true;
                rebuildStructs();
                std::printf("[trackedit] flag post %d on %#x at %.6f (Ctrl+S to save)\n",
                            flagPosts[selFlag].id, circTrack, circFrac);
                beginNaming(NameTarget::Flag, selFlag);
            }
            rebuildOverlay();
            ...
            if (circHit) {
                SimpleEntrySignal e;
                e.id = nextSimpleId++;
                e.name = "SE" + std::to_string(e.id);
                e.trackId = circTrack;
                e.frac = circFrac;
                simpleEntries.push_back(std::move(e));
                selSimple = static_cast<int>(simpleEntries.size()) - 1;
                simpleDirty = true;
                rebuildStructs();
                std::printf("[trackedit] simple entry signal %d on %#x at %.6f "
                            "(Ctrl+S to save)\n", simpleEntries[selSimple].id, circTrack,
                            circFrac);
                beginNaming(NameTarget::Simple, selSimple);
            }
            rebuildOverlay();
        ...
        if (!g_mouseCaptured && mR && !prevMR && mode == EdMode::Circuits) {
            ...
            for (std::size_t i = 0; i < flagPosts.size(); ++i) {
                const glm::dvec3 w =
                    fracToWorld(polys, flagPosts[i].trackId, flagPosts[i].frac);
                if (w.x == 0.0 && w.y == 0.0) continue;
                const glm::vec4 clip = viewProj * glm::vec4(
                    float(w.x - o.x), float(w.y - o.y), float(w.z - o.z) + 2.0f, 1.0f);
                if (clip.w <= 0.0f) continue;
                const glm::vec2 px((clip.x / clip.w * 0.5f + 0.5f) * fbw,
                                   (clip.y / clip.w * 0.5f + 0.5f) * fbh);
                const float dpx = glm::length(px - cur);
                if (dpx < best) { best = dpx; selFlag = static_cast<int>(i); }
            }
            rebuildOverlay();
            ...
            for (std::size_t i = 0; i < simpleEntries.size(); ++i) {
                ...
                const glm::vec2 px((clip.x / clip.w * 0.5f + 0.5f) * fbw,
                                   (clip.y / clip.w * 0.5f + 0.5f) * fbh);
                const float dpx = glm::length(px - cur);
                if (dpx < best) { best = dpx; selSimple = static_cast<int>(i); }
            }
            rebuildOverlay();
        ...
        // HUD: a dark backing panel plus info lines, and a centre crosshair.
        {
          ...
          if (mode == EdMode::Geometry) {
            ...
            const bool haveSel =
                selFlag >= 0 && selFlag < static_cast<int>(flagPosts.size());
            std::snprintf(buf, sizeof(buf), "FLAG POSTS %zu", flagPosts.size());
            appendText(tv, buf, x, 40.0f + 4 * lh, sc,
                       haveSel ? glm::vec3(1.0f, 0.9f, 0.3f) : glm::vec3(0.9f, 0.85f, 0.7f),
                       fbw, fbh);
            if (haveSel && selFlag < static_cast<int>(flagPostStation.size())) {
                // Which station's TXP works it decides whose flag it shows, and that is
                // not visible from the post.
                const SignalStation& ss = flagPostStation[selFlag];
                std::snprintf(buf, sizeof(buf), "%s -> %s (%.0f m)%s   stands %s",
                              flagPosts[selFlag].name.c_str(),
                              ss.name.empty() ? "(no station)" : ss.name.c_str(),
                              ss.distanceM, ss.far ? "  FAR - check this" : "",
                              flagPosts[selFlag].side > 0 ? "right" : "left");
                appendText(tv, buf, x, 40.0f + 5 * lh, sc,
            ...
            appendText(tv,
                       "click track: place   right-click: select   F: side   F2: name   "
                       "X: delete",
                       x, 40.0f + 6 * lh, sc, glm::vec3(0.85f, 0.85f, 0.7f), fbw, fbh);
            ...
            const bool haveSel =
                selSimple >= 0 && selSimple < static_cast<int>(simpleEntries.size());
            // Attachment is what decides which signals interlock with which, so it is
            // worked out live and shown - it is not visible from the mast otherwise.
            simpleEntryStation = attachStations(simpleEntries, stations, polys);
            std::size_t stationCount = 0;
            {
                std::set<std::string> names;
                for (const SignalStation& ss : simpleEntryStation)
                    if (!ss.name.empty()) names.insert(ss.name);
                stationCount = names.size();
            }
            std::snprintf(buf, sizeof(buf), "SIGNALS %zu  at %zu station(s)",
                          simpleEntries.size(), stationCount);
            appendText(tv, buf, x, 40.0f + 4 * lh, sc,
                       haveSel ? glm::vec3(1.0f, 0.9f, 0.3f) : glm::vec3(0.9f, 0.85f, 0.7f),
                       fbw, fbh);
            if (haveSel) {
                const SignalStation& ss = simpleEntryStation[selSimple];
                std::snprintf(buf, sizeof(buf), "%s -> %s (%.0f m)%s   faces %s",
                              simpleEntries[selSimple].name.c_str(),
                              ss.name.empty() ? "(no station)" : ss.name.c_str(),
                              ss.distanceM, ss.far ? "  FAR - check this" : "",
                              simpleEntries[selSimple].dir > 0 ? "+frac" : "-frac");
                appendText(tv, buf, x, 40.0f + 5 * lh, sc,
                           ss.far ? glm::vec3(1.0f, 0.55f, 0.3f)
                                  : glm::vec3(0.8f, 0.95f, 0.8f),
                           fbw, fbh);
            ...
            appendText(tv,
                       "click track: place   right-click: select   F: turn   F2: name   "
                       "X: delete",
                       x, 40.0f + 6 * lh, sc, glm::vec3(0.85f, 0.85f, 0.7f), fbw, fbh);
